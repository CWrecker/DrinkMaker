...
const int enPin = 34;//enable motor high=off, low=on
int count = 0;
...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(2), isrCount, RISING);//set Pin to interupt mode, count on a rising signal from slider stepper
  ...
  attachInterrupt(digitalPinToInterrupt(3), isrMINTCount, RISING);//set Pin 3 to interupt mode, count on a rising signal from from MINT stepper
